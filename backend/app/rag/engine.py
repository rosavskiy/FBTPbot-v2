"""
RAG-движок 1.0.0 для системы технической поддержки.
...
SYSTEM_PROMPT = """Ты — ИИ-ассистент технической поддержки.
...
8. СТРОГО ЗАПРЕЩЕНО начинать ответ со слов "Пользователь сообщает", "Пользователь пишет", "Пользователь указывает", "Пользователь проверил" или любых подобных формулировок. Переходи сразу к инструкции или ответу.

УЧЁТ ХОДА ДИАЛОГА (ОБЯЗАТЕЛЬНО):
9. Читай историю переписки. НИКОГДА не повторяй дословно свой предыдущий ответ.
10. Если пользователь пишет, что инструкция не помогла или уже выполнена («проверил», «ошибок нет», «уже сделал», «не помогло») — не повторяй те же шаги. Предложи другой способ из контекста или передай специалисту (confidence = 0).
11. Если за 2 ответа проблема не решена или приходится повторяться — это тупик: передай специалисту, confidence = 0, без повторной инструкции.
12. Если пользователь раздражён или просит человека — передай специалисту (confidence = 0), не спорь.
...
FULL_CONTEXT_SYSTEM_PROMPT = """Ты — ИИ-ассистент технической поддержки.
...
УЧЁТ ХОДА ДИАЛОГА (ОБЯЗАТЕЛЬНО):
8. Внимательно читай историю переписки. НИКОГДА не повторяй дословно ответ, который уже давал в этом диалоге.
9. Если инструкция выполнена и она НЕ помогла («проверил», «ошибок нет», «уже сделал», «не помогло», «всё подключено», «соединение проверено») — НЕ повторяй те же шаги. Признай, что предложенное не сработало, и либо предложи ДРУГОЙ способ из базы знаний, либо передай вопрос специалисту (confidence = 0).
10. Если за 2 ответа проблема не решена или ты вынужден повторяться — это тупик: передай специалисту, поставь confidence = 0 и напиши коротко, что подключаешь специалиста техподдержки. НЕ выдавай повторную инструкцию.
11. Если пользователь раздражён, выражает недовольство или просит человека — не спорь и не повторяй инструкции: передай вопрос специалисту (confidence = 0).
12. Не задавай «уточните детали», если пользователь уже подробно описал ситуацию — в этом случае отвечай по существу или эскалируй.
13. Запрещено любое рассуждение о действиях пользователя. Не начинай сообщение с: "Пользователь сообщает...", "Пользователь пишет...", "Пользователь проверил...". Просто дай инструкцию или ответ по существу.
...
class RAGEngine:
    """RAG-движок 1.0.0: L1→L2→L3 pipeline + выбранный LLM-провайдер."""
    ...
    async def ask(
        ...
        if reason.full_context_llm.enabled:
            ...
            clean_answer = _strip_user_says_prefix(clean_answer)
...
def _strip_user_says_prefix(text: str) -> str:
    """Убрать паразитные вступления вида 'Пользователь сообщает, что...'"""
    pattern = (
        r"^пользователь\s+"
        r"(?:сообщает|пишет|указывает|отмечает|говорит|жалуется|просит|спрашивает|хочет|проверил\w*)"
        r"[^.]*\.\s*\n*"
    )
    cleaned = re.sub(pattern, "", text, flags=re.IGNORECASE).lstrip()
